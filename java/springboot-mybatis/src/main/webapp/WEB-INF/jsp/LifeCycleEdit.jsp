...
<div id="app" style="background: #FFFFFF;border-radius: 5px;border: none;height: 100%">
    <el-form ref="LifeCycleSvaeForm" :model="form" label-width="100px">
        <%--弹窗一--%>
        <el-dialog :visible.sync="dialogleVisible" size="tiny">
            <el-row :gutter="20">
        ...
        <%--弹窗2--%>
        <el-dialog title="树形目录列表" :visible.sync="dialogTableVisible" size="large">
    ...
    padding-right: 15px;">
        ...
        <div class="form-item-group" style="padding-top: 25px;padding-bottom: 100px;border-bottom: dashed 1px #EEEEEE;border-top: dashed 1px #EEEEEE;">
...
//    import { Message } from 'element-ui';
    ...
    function promptBox() {
        this.$message.error('生命周期阶段,显示优先级,计算优先级不允许重复');
    }
    var rules=new Array();
    $(function() {
        var code_rule;
...
//                    vm.id=getUrlParam("id")
                    return $.ajax({
                        ...
                        dataType: 'json',
                        method: 'POST',
                        data:{"id":vm.id,"page":vm.page,"size":vm.size,"lifeCycleId":getUrlParam("lifeCycleId")}
                    }).done(function(data) {
                        ...
                        vm.data=data.menu;
                        vm.form=data.lifeCycleSaveVO;
                        vm.loading=false
                        rules=vm.form.ruleList;
                        $.each(vm.form.ruleList,function(n,value) {
                            console.log(vm.form.ruleList);
                            console.log(n+': '+value);
//                            alert(n+': '+value);
                            if ($('.idCss').length>0){
                                var index=n+1;
                                ...
                                $('.idCss:last').after("<div class='idCss'><div style='margin-top: 16px;margin-left: 80px;float: left;'><span class='spanId' style='font-size: 14px;color: #48576a;'>"+index+"、</span> </div><div  style='width: 340px;height: 50px;border-radius: 10px;border: 1px solid #bfcbd9;margin-top: 20px;margin-left: 115px;'><button type='button' onclick='deleteDiv(event,"+index+")' style='margin-right: -100px;float: right;margin-top: 6px;' class='el-button el-button--primary el-icon-delete'><!----><!----></button>" +
//                                "<button type='button' style='margin-right: -150px;float: right;margin-top: 6px;' class='el-button el-button--primary'><!----><i class='el-icon-plus'></i><!----></button>" +
                                        " <div style='margin-left: 8px;margin-top: 3px;'><span>属性 : </span><span>"+value.attributeName+"</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <span>运算符 : </span><span>\""+value.ruleOperator+"\"</span> </div>" +
                                        "<div style='margin-left: 8px;;margin-top: 3px;'> <span>值 : </span><span>\""+value.attributeValue+"\"</span> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <span>关系符 : </span><span>\""+value.relationalSymbol+"\"</span> </div>" +
                                        "" +
                                        "</div></div>");
                            }else {
                                console.log("进2"+n);
                ...
                handleClick(index, row) {
                    ...
                    $("#lifeCycleName").html(''+row.attributeName);
                    var vm;
                    vm = this;
                    vm.loading=true;
                    return $.ajax({
...
//                    vm.id=getUrlParam("id")
                ...
                onSubmit(form) {
                    ...
                    var vm;
                    vm = this;
                    form.lifeCycleId=getUrlParam("lifeCycleId");
                    var b=false;
                    return $.ajax({
                        url:"LifeCycleEditForm",
...
//                    vm.id=getUrlParam("id")
                ...
                changeSelect2(data){
                    ...
                    $.each(this.options, function (i, item) {
                        ...
                        if(data==item.value){
                            $("#attributeValue").html(''+item.label);
                        }
                    });
                    if (data!=""){
...
//                    $("#attributeValue").text('"'+data+'"')
                ...
                dialogCancel(){
                    ...
                    $("#lifeCycleName").html("");
                    $("#ruleOperator").html(this.rule.ruleOperator);
                    $("#attributeValue").html(this.rule.attributeValue);
                    $("#relationalSymbol").html(this.rule.relationalSymbol);
                    this.clearable=true;
                ...
                dialogSubmit(){
                    ...
                    var lifeName=$("#lifeCycleName").text();
                    this.rule.attributeName=lifeName;
                    if ($('.idCss').length>0){
...
//                                "<button type='button' style='margin-right: -150px;float: right;margin-top: 6px;' class='el-button el-button--primary'><!----><i class='el-icon-plus'></i><!----></button>" +
                                " <div style='margin-left: 8px;margin-top: 3px;'><span>属性 : </span><span>\"</span><span>"+this.rule.attributeName+"</span><span>\"</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <span>运算符 : </span><span>\""+this.rule.ruleOperator+"\"</span> </div>" +
...
//                        var id='number'+1;
                    ...
                    this.form.ruleList=rules;
                        ...
                        $.each(this.options, function (i, item) {
                            console.log("label:"  + item.label);
                            console.log("value:"  + item.value);
                            if(ruleEntity.attributeValue==item.value){
                                ruleEntity.attributeValue=item.label;
                                console.log("ruleEntity.attributeValue"+ruleEntity.attributeValue);
                            }
                        });
...
Array.prototype.removeArray = function(from, to) {
    ...
    function deleteDiv(event,num){
        console.log(event);
        console.log("num:"+num);
        console.log("rulesSize:"+rules.length);
